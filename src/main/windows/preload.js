...
function init() {
  window.Bridge = {
    ...
    getDomainNameAndBookCodeFromChrome: cmd => {
      let res = execSync(
        'osascript ' +
          path.join(process.env.HOME, '.ShadowWebsocks', 'getDomainNameAndBookCodeFromChrome.applescript')
      ).toString();

      return res.split('\n').filter(it => it.includes('-cloud.chanjet.com'));
    },
    readJson: filename => JSON.parse(fs.readFileSync(filename, {encoding: 'utf-8'})),
